...
//     setTimeout(() => console.log( `a temperatura em Marte é ${callback()} graus Celsius`), messageDelay());
// }
...
const handleError = (errorReason) =>
  ...
  const sendMarsTemperature = (callback) => {
    ...
    const acurate = Math.random() <= 0.6;
            setTimeout(() => {
            if (acurate)  callback(getMarsTemperature());
            else callback(handleError(errorMsg)), messageDelay()
  });
}
